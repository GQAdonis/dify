...
def parse_env_example(file_path):
    """
    Parses the .env file and returns a dictionary with variable names as keys and default values as values.
    """
    env_vars = {}
...
def insert_shared_env(template_path, output_path, shared_env_block, header_comments):
    ...
    # Replace the placeholder shared environment block
    pattern = r"x-shared-env: &shared-api-worker-env(\s+# This block will be populated.*)?"
    if re.search(pattern, template_content, re.DOTALL):
        ...
        # If no placeholder found, start the file with the shared env block
        template_content = f"{shared_env_block}\n\n{template_content}"
...
def main():
    env_file_path = ".env"
    template_path = "docker-compose-traefik-template.yaml"
    ...
    header_comments = (
        "# ==================================================================\n"
        "# WARNING: This file is auto-generated by generate_docker_compose_traefik.py\n"
        "# Do not modify this file directly. Instead, update the .env file\n"
        "# or docker-compose-traefik-template.yaml and regenerate this file.\n"
        "# ==================================================================\n"
    )

    # Check if required files exist
    for path in [env_file_path, template_path]:
        if not os.path.isfile(path):
            print(f"Error: File {path} does not exist.")
            sys.exit(1)

    # Parse .env file
    env_vars = parse_env_example(env_file_path)
